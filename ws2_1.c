...
if(newnode!=NULL)
{
printf("%d  (Bal Fac = %d) | ",newnode->data,balfac(newnode));
...
root=Delete(root,deletemin(root));
printf("\n");
inorder(root);
break;
...
case 11:
printf("\nEnter two keys k1 and k2. ( prints k1, k2 and keys between k1 and k2 ) \n");
scanf("%d %d",&k1,&k2);
printbetween(root,k1,k2);
break;

case 12:
printf("\nNumber of leaf Node(s) : %d . ", count_leaves(root));    
break;
